...
public class PdfPage
{
    ...
    private readonly StringBuilder _contentStream;
    ...
    {
        _contentStream.Append($"BT /{fontAlias} {size} Tf {x} {y} Td ({text}) Tj ET\n");
    }

    public void DrawLine(int x1, int y1, int x2, int y2)
    {
        _contentStream.Append($"{x1} {y1} m {x2} {y2} l S\n");
    }
    ...
    public void DrawImage(string alias, PdfImage image, int x, int y, int width, int height)
    {
        ...
        _contentStream.Append($"q {width} 0 0 {height} {x} {y} cm /{alias} Do Q\n");
    }
    ...
    public void Build(bool compress = false)
    {
        ...
        var rawContent = Encoding.ASCII.GetBytes(_contentStream.ToString());
        byte[] finalContent;
        ...
        if (compress)
        {
            ...
            {
                ds.Write(rawContent, 0, rawContent.Length);
            }
